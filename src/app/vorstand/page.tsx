...
export default function VorstandPage() {
  return (
    <Container size="3" px="0" py={{ initial: "5", sm: "8" }}>
      <Flex direction="column" align="center" gap="2" mb={{ initial: "6", sm: "7" }}>
        ...
        <Heading as="h1" size={{ initial: "7", sm: "8" }} align="center" className="display-title">
          Vorstand
        </Heading>
        <Text size={{ initial: "2", sm: "3" }} color="gray" align="center" style={{ maxWidth: 480 }}>
          Aktuelle Besetzung des Vorstands
        </Text>
      </Flex>

      <Grid columns={{ initial: "2", sm: "3" }} gap={{ initial: "3", sm: "4" }}>
        {vorstand.map((entry) => (
          <Card key={entry.name} size={{ initial: "2", sm: "3" }} className="feature-card panel">
            <Flex direction="column" align="center" gap="3" py="2" style={{ textAlign: "center" }}>
              <div className="person-avatar">{getInitials(entry.name)}</div>
              <Box>
